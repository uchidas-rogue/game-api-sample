locals {
  az_indexed = { for i, az in var.azs : az => i }
}
...
# NAT は 1 台に集約してコストを抑える（AZ 障害時の冗長性は捨てる）。
# 本番では AZ ごとに 1 台ずつにすることを推奨。
...
# Phase 5 で S3 VPC Gateway Endpoint を追加する際はこの route table に紐付ける。
...
resource "aws_security_group" "app" {
  ...
  egress {
    description = "All egress (to ECR/Secrets/CloudWatch via NAT)"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
...
resource "aws_security_group" "db" {
  ...
  description = "Aurora ingress from app"
  vpc_id      = aws_vpc.this.id
