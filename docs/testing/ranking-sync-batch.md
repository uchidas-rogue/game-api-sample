...
| 4 | 正常系: 空テーブル | `…→D→E→F→H→Z` | 正常終了 | Redis SET が1件も呼ばれない |
| 5 | 正常系: スナップショットが反映される | `…→E→F→H→Z` | 正常終了 | `SetGuildScore` / `SetUserPoints` が全件ぶん呼ばれる |
| 6 | Redis SET の一部が失敗 | `…→F→G→F→H→E4` | **エラーを返す** | 1件目が失敗しても**残り全件の SET が呼ばれる**（打ち切らない） |

**エラーの伝搬について**: 1〜3 と 6 はいずれも `errors.Is` で原因のエラーを辿れること
