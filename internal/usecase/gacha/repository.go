// Package gacha のうち、リポジトリインターフェースとトランザクション抽象を定義する。
...
// 本来ここにあるのはおかしいが、現状一つしかないので、一旦ここに置く
// 別機能実装時に、機能横断の共通抽象パッケージへ移動
type Tx interface {
...
// 各メソッドは Tx を引数で受け取ることで、usecase 層がトランザクション境界を
// 制御する設計を実現する。tx は Transactor.DoInTx から渡される値をそのまま透過する。
type Repository interface {
	// GetUserForUpdate はユーザー行を排他ロック付きで取得する。
	// 必ずトランザクション内で呼び出すこと。
	GetUserForUpdate(ctx context.Context, tx Tx, userID int64) (gachadomain.User, error)

	// UpdateUserGems は石残高を更新する。
	UpdateUserGems(ctx context.Context, tx Tx, userID int64, newGems int) error

	// ListItems はアイテムマスタ全件を返す。
	// マスタ参照のためトランザクション必須ではないが、シグネチャを統一するため tx を受ける。
	ListItems(ctx context.Context, tx Tx) ([]gachadomain.Item, error)

	// UpsertUserItem はユーザー所持アイテムを追加（同一item_idは加算）する。
	UpsertUserItem(ctx context.Context, tx Tx, userID int64, itemID int64, num int) error

	// InsertGachaHistory はガチャ履歴を1件追加する。
	InsertGachaHistory(ctx context.Context, tx Tx, userID int64, itemID int64) error
}
