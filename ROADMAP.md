...
* **次の候補:** 指示書側に残る「機械判定できるのに文章で書いている規約」を継続的に `.golangci.yml` / `scripts/` へ移す（[docs/testing/principles/deterministic-verification.md](docs/testing/principles/deterministic-verification.md) §1）。現在の最有力は [docs/testing/README.md](docs/testing/README.md) §6 のレビューゲート——「テスト仕様表のケース番号とテストコードのケース順が一致」「表の行数（重複排除後）とテストケース数が一致」は、Go では計測できない分岐カバレッジをパスカバレッジで代替する仕組みの土台（同 §4）でありながら、現状は**人手のチェックリストにしか置かれていない**。`docs/testing/<機能>.md` の表と対応するテスト関数の `t.Run` 名を突合すれば機械判定できる。次点は AGENTS.md §2「DI 対象のコンストラクタは logger を必須引数とし、nil チェック・`slog.Default()` フォールバックを実装しない」で、`slog.Default()` の呼び出し自体は forbidigo が禁止済みだが「必須引数になっているか」「nil チェックを書いていないか」は未判定
